...
class DBXMetaGenApp:
    ...
    def setup_client(self):
        ...
        try:
            # Use service principal authentication (M2M OAuth)
            # The service principal credentials are set in the app environment by the asset bundle
            host = os.environ.get("DATABRICKS_HOST")

            if host:
                # Use default authentication (service principal M2M OAuth)
                st.session_state.workspace_client = WorkspaceClient(host=host)
            ...
            with col2:
                if st.button("🔍 Validate Tables", type="secondary"):
                    if table_names_input.strip():
                        tables = [
                            t.strip()
                            for t in table_names_input.split("\n")
                            if t.strip()
                        ]
    ...
    def render_job_management(self):
        ...
        with col1:
            ...
            if tables_source == "Manual Input":
                ...
                tables_list = (
                    [t.strip() for t in table_names_for_job.split(",") if t.strip()]
                    if table_names_for_job
                    else []
                )
            else:
                ...
                if tables_list:
                    ...
                    st.warning(
                        "⚠️ No tables loaded from CSV. Please upload a CSV file first."
                    )
        ...
        with col2:
            ...
            if st.button(
                "🚀 Create & Run Job", type="primary", disabled=not tables_list
            ):
        ...
        try:
            with st.spinner("🚀 Creating and starting job..."):
                ...
                job_config = JobSettings(
                    ...
                    email_notifications={
                        "on_failure": [
                            st.session_state.workspace_client.current_user.me().user_name
                        ]
                    },
                )
                ...
                st.success(
                    f"✅ Job created and started! Job ID: {job.job_id}, Run ID: {run.run_id}"
                )
            ...
            with st.spinner("🔄 Refreshing job status..."):
                for run_id, job_info in st.session_state.job_runs.items():
                    ...
                    if run_details.end_time:
                        job_info["end_time"] = datetime.fromtimestamp(
